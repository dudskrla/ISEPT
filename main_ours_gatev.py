...
#    python3 main_ours_gatev.py --start_year 1991 --start_month 2
...
# argparse 설정 및 상수 정의
# ──────────────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="CAE+MLP 기반 페어 선정 및 Gatev 트레이딩 시뮬레이션")
...
# Gatev 임계값 설정 (Z-score 방식; dev = spread - mu)
...
BEST_PAIR_DIR = f"./BEST_PAIR/TRAIN_ONE/2_GATEV/{INIT_YEAR}"
OUTPUT_DIR    = f"./PAIR_RESULT/TRAIN_ONE/2_GATEV/{INIT_YEAR}"
...
def load_price_dict(tickers: list, months: list, base_dir: str, field='CLOSE') -> dict:
    ...
    데이터가 없는 경우 빈 Series 또는 NaN으로 채워진 Series를 반환합니다.
    """
...
# ──────────────────────────────────────────────────────────────────────────────
# Gatev 트레이딩 시뮬레이터 (Trading Simulator)
# ──────────────────────────────────────────────────────────────────────────────
def simulate_pair_gatev(pa: pd.Series, pb: pd.Series, mu: float, sigma: float,
                        k_entry=2.0, k_exit=0.0):
    """
    Gatev 스타일(Z-score Spread)의 페어 트레이딩을 시뮬레이션합니다.

    Args:
        pa, pb (pd.Series): 종목 A, B의 가격 시계열
        mu (float): Spread의 평균 (Formation 기간 기준)
        sigma (float): Spread의 표준편차 (Formation 기간 기준)
        k_entry (float): 진입 임계값 (기본 2.0 sigma)
        k_exit (float): 청산 임계값 (기본 0.0 sigma - 평균 회귀)

    Returns:
        tuple: (perf_dict, daily_nav, event_df)
    """
    pm = Pair_Trading()
    events = []
    pos_side = None
    ...
    for dt in dates:
        ...
        # 1. 상장 폐지(Delisting) 또는 데이터 결측 처리
        ...
        if pending:
            ...
            if side == 'open':
                ...
                pos_side = +1 if (L == 'A' and S == 'B') else -1
        ...
        if pos_side is None:
            ...
            if abs_dev >= entry:
                if dev > 0:
                    # A가 고평가 -> Short A, Long B
                    pending = ('open', ('B', 'A', b, a))
                else:
                    # A가 저평가 -> Long A, Short B
...
# ──────────────────────────────────────────────────────────────────────────────
def run_pair_trading_gatev_like(pair_list, prior_y, prior_m, ym_tag, base_ohlc_dir, out_dir, out_daily_dir,
                                k_entry=2.0, k_exit=0.0):
    """
    선정된 페어 리스트에 대해 월별 Gatev 트레이딩을 수행합니다.
    
    - Formation: 전월(prior) 기준 과거 12개월
    - Trade: 전월(prior) 기준 미래 6개월 (이번 달 포함)
    ...
    for (p1, p2) in tqdm(pair_list, desc=f"Trade {ym_tag} (Gatev)"):
        # Formation 기간 통계 산출
        ...
        idx_f = s1_form.index.intersection(s2_form.index).sort_values()
        if idx_f.empty:
            continue

        spread_f = s1_form.loc[idx_f] - s2_form.loc[idx_f]
        ...
        # 시뮬레이션
        perf, daily_nav, _ = simulate_pair_gatev(pa, pb, mu, sigma, k_entry=k_entry, k_exit=k_exit)
...
# ──────────────────────────────────────────────────────────────────────────────
def make_pairs_csv_for_month(
    month_str: str,
    df_univ: pd.DataFrame,
    ohlc_dir: str,
    out_dir: str,
    top_candidates: int = 5000,   # SSD로 뽑을 후보 페어 수
    min_days: int = 120,          # Formation 최소 겹치는 일수
    k_entry: float = 2.0,
    k_exit: float = 0.0,
):
    """
    초기 학습을 위한 라벨 데이터가 없을 때, SSD(Sum of Squared Differences) 방식으로
    후보 페어를 선정하고 Sharpe Ratio를 계산하여 라벨 파일(pairs.csv)을 생성합니다.
    ...
    Args:
        ...
        out_dir (str): 저장 경로
        top_candidates (int): 1차 필터링할 후보 수
    ...
    months_form  = last_n_months(prior_y, prior_m, 12)  # (끝=month_str-1)
    months_trade = next_n_months(prior_y, prior_m, 6)   # (시작=month_str+1)
    ...
    # SSD 계산 (Gatev Classic 방식): G = X^T * X 이용
    G = arr.T @ arr
    diag = np.diag(G)
    ...
    cand_pairs.sort(key=lambda x: x[2])  # SSD 오름차순 정렬

    # Trade 기간 시뮬레이션 및 라벨링 (Sharpe Ratio)
    pt = load_price_dict(tickers, months_trade, ohlc_dir, field="CLOSE")

    rows = []
    for a, b, ssd_score in tqdm(cand_pairs, desc=f"Bootstrap pairs {month_str}"):
        sa_f = X[a]
        sb_f = X[b]
        spread_f = sa_f - sb_f
        mu = float(spread_f.mean())
    ...
    if df_out.empty:
        raise ValueError(f"[{month_str}] failed to generate any labeled pairs")

    df_out.sort_values("sharpe", ascending=False, inplace=True)
    df_out.to_csv(out_path, index=False)
    print(f"[BOOTSTRAP] Saved pairs csv -> {out_path} (rows={len(df_out)})")
    return out_path


def _load_pairs_for_period(period: str):
    """
    저장된 'best_pair_list.csv'에서 페어 리스트를 로드합니다.
    호환성을 위해 다양한 컬럼 형식을 지원합니다.
    """
    p = f"{BEST_PAIR_DIR}/{period}/best_pair_list.csv"
    if not os.path.exists(p):
        return None
    df_pairs = pd.read_csv(p)
    
    # 1991-02 등 초기 데이터 포맷 대응
    if period == "1991-02":
        return [(row['ticker1'], row['ticker2']) for _, row in df_pairs.iterrows()]
    
    if 'pair' in df_pairs.columns:
        pairs = [row['pair'].split("_") for _, row in df_pairs.iterrows()]
        return [(a, b) for a, b in pairs]
    
    # ticker1/ticker2 컬럼 fallback
    if 'ticker1' in df_pairs.columns and 'ticker2' in df_pairs.columns:
        return [(row['ticker1'], row['ticker2']) for _, row in df_pairs.iterrows()]
    return None
...
def main():
    ...
    for offset in range(1, TRAIN_MONTHS):
        ...
        prior_period = dt_prior.strftime("%Y-%m")
        ...
        pair_list = _load_pairs_for_period(period)

        if pair_list is None:
            # 이전 달의 결과(라벨) 경로 설정
            prev_res_month = prev_month_str(prior_period)
            ...
            if not os.path.isfile(prev_results_path):
                # ✅ 첫 실행(또는 결과 부재) 시 Bootstrap(Warm-start) 수행
                warmstart_pairs_path = make_pairs_csv_for_month(
                ...
                prev_results_path = None  # Warm-start로 대체
            ...
            pair_list = run_mlp_pipeline(
                ...
                # 논문 구현: 이전 트레이딩 결과가 다음 학습의 라벨이 됨
                prev_results_path=prev_results_path,
                warmstart_pairs_path=warmstart_pairs_path,
            )

        # ─── [1st] 트레이딩 시뮬레이션 (Gatev) ───
        print(f"[1st] 페어 트레이딩 (Gatev): {period}")
        run_pair_trading_gatev_like(
            pair_list=pair_list,
        ...
        if not os.path.exists(f"{CAE_MODEL_DIR}/{period}/best.pth"):
        ...
        # 저장 공간 확보를 위해 CAE 이미지 폴더 정리
        folder_path = f"./DATA/IMAGE_CAE/{prior_period}"
    ...
    for offset in range(TRAIN_MONTHS, TRAIN_MONTHS + TEST_MONTHS):
        ...
        # 테스트 기간 트레이딩
        print(f"[1st-TEST] 페어 트레이딩 (Gatev): {period}")
        run_pair_trading_gatev_like(
            pair_list=pair_list,
            prior_y=prior_y, prior_m=prior_m,
            ym_tag=period,
